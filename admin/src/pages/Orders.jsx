...
function Orders() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "Refund Processing": return "text-yellow-300";
      case "Refunded": return "text-green-300";
...

          {
            orders.map((order, index) => (
              <div key={index} className='w-[90%] h-[40%] bg-slate-600 rounded-xl flex lg:items-center items-start justify-between flex-col lg:flex-row p-[10px] md:px-[20px] gap-[20px]'>
                ...
                {/* ITEMS */}
                <div>
                  <div className='flex items-start flex-col gap-[5px] text-[16px] text-[#56dbfc]'>
                ...
                <div className='text-[15px] text-green-100'>
                  ...
                  {/* STATUS COLOR */}
                  <p className={`text-[18px] font-semibold ${getStatusColor(order.status)}`}>
                    {order.status}
                  </p>
                ...
                >
                  ...
                  {/* NEW STATUS FOR CANCELLED OR REFUND */}
